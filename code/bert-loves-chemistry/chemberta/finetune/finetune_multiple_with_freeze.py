...
def finetune_model_on_single_dataset(
    ...
    config = RobertaConfig.from_pretrained(checkpoint_dir, **config_kwargs)
    ...
    def warmup_model_init():
        ...
        load_kwargs = {"config": config, "state_dict": state_dict}
        if hf_token:
            load_kwargs["use_auth_token"] = hf_token
        model = model_class.from_pretrained(checkpoint_dir, **load_kwargs)
    ...
    def hp_model_init():
        ...
        if hf_token:
            load_kwargs["use_auth_token"] = hf_token
        model = model_class.from_pretrained(warmup_model_dir, **load_kwargs)
