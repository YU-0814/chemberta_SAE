...
from chemberta.utils.hf import get_hf_token
...
flags.DEFINE_enum(
    name="model_impl",
    default="hf",
    enum_values=["hf", "custom"],
    help="Select the base model implementation.",
)
...
def finetune_single_dataset(dataset_name, dataset_type, run_dir, is_molnet):
    ...
    tokenizer = RobertaTokenizerFast.from_pretrained(
        FLAGS.tokenizer_path, max_len=FLAGS.max_tokenizer_len, **tokenizer_kwargs
    )

    finetune_datasets = get_finetune_datasets(dataset_name, tokenizer, is_molnet)

    if FLAGS.pretrained_model_name_or_path:
        config_kwargs = {"use_auth_token": hf_token} if hf_token else {}
        config = RobertaConfig.from_pretrained(
            FLAGS.pretrained_model_name_or_path, **config_kwargs
        )
    else:
    ...
    base_model_cls = None
    if FLAGS.model_impl == "custom":
        from chemberta.custom_roberta import CustomRobertaModel

        base_model_cls = CustomRobertaModel
    model_kwargs = {"base_model_cls": base_model_cls} if base_model_cls else {}
