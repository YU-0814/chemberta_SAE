...
def create_trainer(
    ...
    base_model_cls = None
    model_cls_mlm = RobertaForMaskedLM
    if model_impl == "custom":
        from chemberta.custom_roberta import CustomRobertaForMaskedLM, CustomRobertaModel

        model_cls_mlm = CustomRobertaForMaskedLM
    ...
    if pretrained_model:
        load_kwargs = {"config": config, **model_kwargs}
        if hf_token:
            load_kwargs["use_auth_token"] = hf_token
        model = model.from_pretrained(pretrained_model, **load_kwargs)
    else:
